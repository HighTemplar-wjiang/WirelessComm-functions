...
function [xHat, h] = mmse_bpsk(bit_seq, snr, nTx, nRx, sic_flag, opt_ord_flag)
    ...
    % Compute W = (H'H + N0I)^-1*H'
    w = zeros(nRx, sym_len);
    for idx = 0:1:(sym_len/nTx)-1
        
        hi = h(:, :, idx+1);
        wi = pinv(hi'*hi + 1/ebn0 * eye(min([nTx nRx])))*hi';
    ...
    if (sic_flag ~= 0) % SIC
        ...
        uMat  = repmat(eye(min([nTx nRx])), [1 sym_len/nTx]);
        for ii = 1:1:nTx
            ...
            for idx = 0:1:(sym_len/nTx)-1
                ...
                hi = hCan(:, :, idx+1);
                wi = pinv(hi'*hi + 1/ebn0 * uMat(:, 1 + idx*nTx:(idx+1)*nTx))*hi';
